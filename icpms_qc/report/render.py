...
from icpms_qc import __version__
from icpms_qc.model import Batch
from icpms_qc.qc.checks import CheckResult, Outcome
from icpms_qc.qc.engine import verdict as _verdict
...
def to_json_dict(batch: Batch, results: list[CheckResult]) -> dict:
    return {
        ...
        "generated_by": f"icpms-qc {__version__}",
...
def to_html(batch: Batch, results: list[CheckResult]) -> str:
    ...
    parts.append(f"<!doctype html><html><head><meta charset='utf-8'>"
                 f"<title>icpms-qc report — {_esc(Path(batch.source_path).name)}</title>"
                 f"<style>{_CSS}</style></head><body><main>")
    parts.append(f"<h1>icpms-qc QC report {overall_badge}</h1>")
    parts.append(
        ...
        f"&middot; generated: {_esc(_now_iso())} by icpms-qc {_esc(__version__)}</div>")

    if batch.warnings:
        items = "".join(f"<li>{_esc(w)}</li>" for w in batch.warnings)
        parts.append(f"<div class='warnbox'><b>Parser warnings</b><ul>{items}</ul></div>")

    # Analytes as icpms-qc understood them. A reviewer's fastest way to catch a
    ...
    parts.append(
        "<footer>Generated by <b>icpms-qc</b> (MIT) &middot; "
        "<a href='https://github.com/yzoe236/icpms-qc'>github.com/yzoe236/icpms-qc</a>"
